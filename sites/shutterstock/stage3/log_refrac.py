    ...
    while True:
        ...
        while batches_sent != 0:
            ...
            if batch['meta']['aw_worker_index'] != index:
                raise Exception(f"aw-{index}: {vid_id} - batch {batch['meta']['batch_id']} received from wrong worker, got {batch['meta']['aw_worker_index']}, expected {index}")
...
if __name__ == "__main__":
    ...
    # new out_data pipe scheme:
    #  - each worker has its own out_data pipe
    #  - an additional process reads the main out_data_pipe (connected to tpu_worker) and distributes the data to the worker out_data pipes based on worker_id
    ...
    if USE_WANDB:
        ...
        wandb_worker_process = mp.Process(target=wandb_worker_func, args=(wandb_pipe, tmp_c, file_pipe, in_data_pipe, tpu_out_data_pipe, tar_pipe,))
    ...
    pipe_manager_process = mp.Process(target=out_pipe_manager, args=(tpu_out_data_pipe, out_data_pipes,))

    assign_worker_processes = list()
    for i in range(ASSIGN_WORKER_COUNT):
        assign_worker_process = mp.Process(target=assign_worker_func, args=(i, file_pipe, in_data_pipe, out_data_pipes[i], tar_pipe,))
